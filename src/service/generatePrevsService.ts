...
export default class GeneratePrevsService {

	public static async generateAlbumPreviews(album: IAlbum, recreate: boolean = false): Promise<any> {

		try {

			C.logI(`generateAlbumPreviews for albumId ${album.id}`);
	...
	private static async generatePreviewsForGroup(list: any[], folder: string, prevFolder: string, thumbFolder: string): Promise<any> {
		await Promise.all(
	...
	private static async generatePhotoPreviews(file: string, folder: string, prevFolder: string, thumbFolder: string) {
